...
class MusicProvider(Provider):
    ...
    async def sync_library(self, media_type: MediaType) -> None:
        ...
        if self.library_sync_deletions_enabled():
            # query the database directly for all items with in_library=True
            # for this provider instance, instead of relying on the cache
            # which can be lost on cache clear/expiry/migration
            ...
            prev_library_items: set[int] = {int(row["item_id"]) for row in db_rows}
